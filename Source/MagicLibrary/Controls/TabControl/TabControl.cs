...
namespace Crownwood.Magic.Controls
{
    ...
    public class TabControl : Panel
    {
        ...
        protected TabPageCollection _tabPagesInVisible;	// collection of pages that are not Visible
        ...
        public TabControl()
        {
            ...
            _tabPagesInVisible = new TabPageCollection();
                ...
                if (xDraw < xMax)
                {
                    ...
                    if (xDraw < xMax)
                    {
                       ...
                       // g.DrawString(page.Title, drawFont, drawBrush, drawRect, drawFormat);
                        g.DrawString(page.Title, page.TitleFont, drawBrush, drawRect, drawFormat);
                    }
        ...
        protected virtual void OnPageVisibleChanged(object sender,EventArgs e)
        {
            if (sender is TabPage)
            {
                ...
                {
                    //
                    // Make sure this is not already in our list of items disabled...
                    //
                    if (_tabPagesInVisible.Count != 0)
                    {
                        if (_tabPagesInVisible.Contains((TabPage)sender))
                        { return; }
                    }

                    if (_tabPages.Contains((TabPage)sender))
                    {
        ...
        protected virtual void HandleShowingTabPage(TabPage page, Control c)
        {
            ...
            if (!page.Shown)
            {
                ...
                if ((f != null) && (f.AutoScaleMode != AutoScaleMode.None))  // foo: was  && (f.AutoScale))
                {
